...
while read enimi pnimi grupp; do
	...
	#Kontrollin kas grupp on olemas kuhu kasutaja tahan lisada
	cat /etc/group | grep $grupp >> /dev/null
	if [ $? -ne 0 ]; then
		groupadd $grupp
	fi

	#Kontrollinn, ega ei ole süsteemis loodavat kasutajat olemas
	...
	if [ $? -ne 0 ]; then
		...
		echo "Süsteemis on juba kasutaja nimega $kasutajanimi"
	fi
